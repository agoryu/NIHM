...
public class DTW {
	...
	/** Calcul du score de la reconnaissance de forme
	 * @return
	 */
	public double score(final int j) {
		
		double result = 0.0;
		double g[] = new double[n];
		for(int t=0; t<n; t++) {
			g[t] = Math.pow(RStroke.get(t).x - RStroke.get(t).y, 2);
		}
		return result;
	}
	
	/** Calcul de la moyenne
	 * @param tab tableau de distance
	 * @param indexI index i au moment t
	 * @param indexJ index j au moment t
	 * @return la moyenne
	 */
	private double moyenne(final double [][] tab, final int indexI, final int indexJ) {
		double result = 0.0;
	 ...
	 * @param indexI index i au moment t
	 * @param indexJ index j au moment t
	 * @param moyenne moyenne au moment t
	 * @return la variance
	 */
	private double variance(final double [][] tab, final int indexI, final int indexJ, final double moyenne) {
		
		double result = 0.0;
		for(int i=0; i<indexI; i++) {
			for(int j=0; j<indexJ; j++) {
				result += Math.pow(tab[i][j] - moyenne, 2);
			}
		}
		return result / (indexI + indexJ);
	}
